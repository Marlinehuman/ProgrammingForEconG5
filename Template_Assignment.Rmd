...
**1. Economic relevance.** The housing market is a cornerstone of the economy. Rising housing costs reduce disposable income, limit labor mobility, and contribute to wealth inequality between homeowners and renters (CBS, 2025). Geographic differences in housing pressure also shape patterns of regional economic development. This pattern has also been identified at the national level by spatial planning research, which points to liquidity constraints and regional mismatches between housing supply and demand as key structural drivers (Planbureau voor de Leefomgeving, 2024).
...
Urban density is measured through `ste_mvs` (urbanity code, ranging from 1 = very strongly urban to 5 = non-urban) and `bev_dich` (population density per km²). Property values are captured through `g_wozbag` (average WOZ value in euros), which serves as a proxy for housing market prices.
...
# Installing packages which will be used. ----
  ...
  clean_year <- function(raw_csv, year_val) {
    # Lees alles eerst als text (character) om fouten te voorkomen
    df <- read_csv(raw_csv, 
                   na = c(".", "", "NA", " "), 
                   show_col_types = FALSE,
                   col_types = cols(.default = col_character()))  # Alles als text
    
    # Lijst van kolommen die numeriek zouden moeten zijn
    numeric_cols <- c("a_woning", "a_hh", "p_leegsw", "g_wozbag", "p_koopw", 
                      "p_huurw", "p_wcorpw", "p_ov_hw", "ste_mvs", "bev_dich", "a_inw")
    
    # Zet deze kolommen veilig om naar numeriek
    ...
    # WOZ waarde omzetten van duizenden euros naar euros
    if("g_wozbag" %in% names(df)) {
...
First, we filtered the dataset to municipality-level observations only (recs == "Gemeente"), excluding neighbourhood and district-level rows. This choice was made because municipalities are the most relevant unit for housing policy analysis — they are the administrative bodies responsible for zoning, construction permits, and social housing allocation. The trade-off is that we lose within-municipality variation: a municipality with high average housing pressure may still contain neighbourhoods with severe vacancy, which our analysis cannot detect.

Second, we selected 15 variables from the original dataset of over 100 columns. The selected variables cover housing stock, vacancy, ownership structure, urbanity, population density, and property values — the core dimensions needed to quantify housing pressure and the housing paradox. The trade-off here is that variables potentially relevant to housing (such as income or migration data) were excluded to keep the analysis focused and the dataset manageable, which means our model cannot account for these confounding factors.

Third, the WOZ value variable (`g_wozbag`) was multiplied by 1000 to convert it from thousands of euros to euros, ensuring the unit is interpretable and consistent with other monetary variables. This is a unit conversion rather than a substantive trade-off.

One limitation of this cleaning approach is that municipalities change over time due to mergers. Some municipalities present in 2020 may not exist under the same name in 2025, which could introduce small inconsistencies in the longitudinal analysis. Because we use direct name matching rather than an official municipality concordance table, a small number of merged municipalities may not perfectly align across years, which slightly reduces the effective sample size for time-series comparisons.
...
We chose multiplication rather than addition to combine housing pressure and vacancy because the housing paradox is specifically about *coexistence*: a municipality with high pressure but zero vacancy, or high vacancy but no pressure, does not represent the paradox we are studying. Multiplication ensures the index is only high when both conditions are present simultaneously. Because housing pressure typically ranges between 0.8 and 1.3 and vacancy rate is a percentage (typically 2–10), the resulting paradox index has no fixed theoretical maximum, but in practice ranges roughly between 1 and 22 in our data, with higher values indicating more severe coexistence of shortage and unused capacity.
...
```{r subpopulation_table, echo=FALSE}
mdf %>%
